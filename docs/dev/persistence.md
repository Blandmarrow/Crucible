...
| `GALLERY_RATING_BADGE_KEY` | `localStorage`, `"true" \| "false"`, **on** by default | Also owned by `uiPrefsStore` (`getGalleryRatingBadge()`), never read directly by a component | Per-card keep/cut rating badge, same `!== "false"` read as the style meter above. Unlike it there is no request to gate — the rating ships in the list payload regardless — so this is a render toggle only. See `docs/dev/rating.md` |
