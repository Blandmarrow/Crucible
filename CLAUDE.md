...
| `docs/dev/gallery-nav.md` | Working on the detail view's arrows, `gallery-nav-*`, `injectNavId`/`paneGo`, or the live refresh while a job adds rows — the stored context and `navPageParams`/`navIdsParams`, the two boundary prefetch queries, `atEnd` and the post-delete re-derivation, the live total | ~2120 |
