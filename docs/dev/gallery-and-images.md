...
The `GalleryPage` toolbar also carries an **"Import folder" button** (next to Rescan/Upload) that opens the shared `ImportFolderModal` for the current dataset, so folder import is reachable without returning to `DatasetsPage`. See `docs/dev/dashboard-pages.md` § Datasets page for `ImportFolderModal` and the native folder-dialog Browse button.
