...
| `docs/dev/dashboard-pages.md` | Datasets page (categories, duplicate, import), Statistics page (histograms, CSV export, BucketPanel), Settings page (tabs, thresholds), hardware stats, file browser | Working on `DatasetsPage`, `StatsPage`, `SettingsPage`, hardware meters, or `FileBrowserPage` |
| `docs/dev/frontend-core.md` | TanStack Query/Zustand conventions, SSE hooks, job-completion cache invalidation, shared constants modules, Sidebar/Layout, split-view pane manager, Tailwind/CSS design system | Working on global frontend state, a new job-triggering UI, the pane/split-view system, or styling |
