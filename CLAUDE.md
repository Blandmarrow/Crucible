...
| `docs/dev/image-files.md` | Image naming/renaming/collisions, cross-dataset move and copy, folder import, rescan/sync, standalone caption import, drag-`.txt`-onto-image | Working on image upload/move/copy/rename or caption import | ~1570 |
...
| `docs/dev/video.md` | `Video` model and `videos/` layout, `media_types.py` allowlist, cv2 metadata ladder and its duration guard, poster generation and lazy backfill, video ingest via upload/folder-import/rescan, the `/videos` endpoints incl. rename and range serving, `VideoStrip`/`VideoDetailPage` | Working on videos, poster frames, frame extraction, or any file-extension allowlist | ~3490 |
...
| `docs/dev/frontend-core.md` | TanStack Query/Zustand conventions, SSE hooks, job-completion cache invalidation, `errorConsoleStore`/`ErrorConsole`, shared constants modules, Sidebar/Layout, split-view pane manager, route-level code splitting | Working on global frontend state, a new job-triggering UI, panes, lazy page loading, or the JS error console | ~3340 |
| `docs/dev/persistence.md` | `constants/storage.ts` key registry, `loadPersisted`/`useDebouncedPersist`, the three persistence shapes, the workflow/filters persistent page state pattern | Adding a storage key or persisting page configuration | ~1480 |
