...
**License badge**: the module-level `LicenseBadge({ ds })` in `DatasetsPage.tsx` renders the dataset's *default* license (`DatasetOut.license`) via `licenseInfo()` from `constants/licenses.ts` — card footer (right-aligned, next to the captioned count and the optional `source_name`) and compact row alike. Unlike the per-image gallery badge (`ImageCard`, opt-in via Settings → Gallery, hidden when empty) this one **always renders**, falling back to the muted "No license" descriptor: there is one badge per dataset rather than per image, so it is not noise, and an unset default is exactly the state the badge exists to surface. The `title` composes the label with source name, non-commercial, and attribution-required notes. See `docs/dev/gallery-and-images.md` § Source & license provenance for the inheritance model.
