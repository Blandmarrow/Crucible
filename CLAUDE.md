...
| `docs/dev/gallery.md` | Working on `GalleryPage`, the subfolder sidebar, or a gallery filter — selection and shift-click ranges, select-all-matching-filters, subfolder create/rename/move/delete and the row context menu, expand/collapse and its persistence, filters, pagination, sorting | ~5290 |
...
| `docs/dev/image-detail.md` | Working on `ImageDetailPage`, the style-match block or generation-metadata display — gallery/nav persisted keys, `injectNavId`/`paneGo`, the post-delete nav refresh, crop tool, selection toggle, caption panel, `StyleMatchPanel` and `DinoLayerBreakdown` | ~3165 |
