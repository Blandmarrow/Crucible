...
This file covers reading and curating a gallery in `GalleryPage`: image selection and the shift-click range model, the subfolder sidebar and its rename/move operations, the filter controls, and sorting. Everything dnd-kit — manual drag ordering, dragging image cards onto subfolder rows, and dragging a subfolder onto another — is in `docs/dev/gallery-dnd.md`. The detail view is in `docs/dev/image-detail.md`, what this page persists between visits in `docs/dev/gallery-state.md`, and the navigation context its arrows walk in `docs/dev/gallery-nav.md`; file naming, imports and rescan in `docs/dev/image-files.md`.
...
**The toolbar is two flex groups, not one wrapping row**, so adding a filter can no longer
displace the Import/Rescan/Upload cluster: a filter group (`flex: 1 1 700px`, `minWidth: 0`,
ending with **Reset filters**) and an action group (`marginLeft: auto`, `flex: 0 0 auto`).
Both numbers matter. `minWidth: 0` lets the filters shrink instead of shoving the actions off
the row. The 700 px basis decides the *line break* — flex breaks lines on the basis, not on
the grown width — so a smaller one let the actions hold row 1 at every width and squeezed the
filters into a 400 px column four rows deep beside a blank band. **Reset filters** is a
sibling of the score-chip block rather than a child, or a wrapping row of chips drags it
around.

- **Search bar** — debounced 350 ms; passes `search` param to `GET /images/`; filters by filename OR caption text (case-insensitive); has a clear (×) button when set, which clears **both** halves of the debounced pair synchronously and calls `resetPage()`. State is the committed `search` (never the `searchInput` draft), persisted in `gallery-state-${datasetId}` alongside `licenseFilter` and cleared by **Reset filters**. A restored value seeds `searchInput` *and* `search` from one local — see `docs/dev/gallery-state.md` § The debounce rules for why seeding one half alone re-creates PM-012.
...
- **Score filters** — multi-chip system: each active filter is a `{field, min?, max?}` chip with a × remove button. An "Add score filter" form lets the user pick any of the 9 score fields and enter optional min/max bounds. Multiple chips are combined as AND conditions via the JSON-encoded `score_filters` param. The older single `score_field`/`min_score`/`max_score` params are not used by GalleryPage (retained only for StatsPage BucketPanel backward compat).
  ...
  Persisted in `gallery-state-${datasetId}` alongside `licenseFilter` and cleared by **Reset filters**. What is stored is the **chip array as the component holds it** — string `min`/`max`, where `""` means "no bound" and is not `0` — never `scoreFiltersParam`, whose `parseFloat` pass has already collapsed that distinction (and which is a JSON string already, so storing it would re-introduce the double-encoding `utils/galleryNav.ts` warns about). On read it goes through the module-local `sanitizeScoreFilters`: an array, each entry an object whose `field` is in `SCORE_FIELDS` and whose bounds are `""` or survive `parseFloat`, with bound-less entries dropped because `applyScoreFilter` structurally cannot create one. The failure it guards is **a lying chip, not an empty grid** — unlike an unknown `licenseFilter` id, an unrecognised score field is skipped by the backend's decode loop, whose `except … pass` then abandons the rest of the list, so the bad chip filters nothing *and* silently stops the chips after it from applying. Unlike the label bounds check below, this runs in the `useState` **initializer** rather than during render: `SCORE_FIELDS` is compiled in, so "valid at mount" is "valid forever", whereas the label vocabulary is fetched and can change mid-session. A render-phase version would also need an exact is-anything-invalid loop guard (the sanitizer returns a fresh array each call) and its `setPage(1)` + `dropScrollRestore()` companions would re-create the restore loss persisting these chips exists to remove.
- **Detection label** — text input with icon prefix, debounced 350 ms; passes `detection_label` to `GET /images/`; uses a correlated `EXISTS` subquery against the `detections` table matching `label ILIKE '%...%'`; has a clear (×) button when set, clearing both halves of the pair. Same shape and same seeding rule as the search bar above: the committed `detectionLabel` is persisted in `gallery-state-${datasetId}`, cleared by **Reset filters**, and restored into `detectionLabelInput` as well as `detectionLabel`.
...
- **Label filter** — a `LabelPicker` dropdown (`docs/dev/styling.md` § Anchored popovers) over the vocabulary (from `useLabels`, one app-wide cache entry), each row carrying the dataset-scoped count from `GET /labels/counts`, with an **Any/All** segmented toggle that only appears once two or more are ticked and an **Unlabelled** toggle in the panel footer. It replaced a chip per vocabulary entry, which cost the toolbar permanent width for every label anyone ever defined; the collapsed trigger summarises what is on (`name +N`, `·All` for the intersection, `Unlabelled`) and is outlined in `--accent` while engaged, since a filter behind a closed panel is otherwise invisible. The whole control is hidden when the vocabulary is empty, so the filter bar gains nothing until a label exists. Ticking a label clears **Unlabelled** and vice versa — the backend 400s on the unsatisfiable pair. The picker is a pure view: every semantic below lives in `GalleryPage`. State is `labelFilter: string[]` / `labelMatch` / `labelMissing`, persisted in `gallery-state-${datasetId}` alongside `licenseFilter` (which means **six** edits, see `docs/dev/labels.md` § Frontend), cleared by **Reset filters**, and bounds-checked against the vocabulary so an id whose label was deleted is dropped rather than silently showing an empty grid — the `licenseFilter` precedent. That check is derived **during render**, gated on `useLabels().isLoaded` (the `frameVideoId` guard below is the same shape), not latched behind a mount-once ref: a ref never reconciled an empty vocabulary, and never reconciled a label deleted later in the session. Ids, not names, because the blob outlives a rename. See `docs/dev/labels.md`.
