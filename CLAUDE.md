...
| `docs/dev/image-similarity.md` | Working on duplicate detection, pHash grouping or bulk duplicate resolution — `find_duplicates_sync`'s two implementations, same-source duplicate groups, the duplicates panel's filter chips/bulk bar/batched resolve, the card layout | ~2305 |
| `docs/dev/style-similarity.md` | Working on style similarity, the style-run descriptor or the style-match meter — the `embedding_type` modes and what each is measurably worth, all-layers DINOv2 scoring, `StyleSimilarityRun` and `GET /quality/style-similarity/{id}`, the percentile contract, the card meter and the detail block | ~2725 |
