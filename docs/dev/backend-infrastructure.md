...
**SAM2 install step**: after `pip install -r requirements.txt`, both `setup` (step 5/6) and `update` (step 3/5) prompt (`[Y/n]`, default Yes) to `pip install git+https://github.com/facebookresearch/sam2.git pycocotools`. Declining or a failed install only disables Grounded SAM2 segmentation (`backend/ml/sam2_predictor.py`); all other features continue to work. Setup is now 6 steps and update 5 steps — both renumbered from the SAM2 addition.
