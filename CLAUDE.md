...
| `docs/dev/image-files.md` | Image naming/renaming/collisions, cross-dataset move and copy, folder import, rescan/sync, standalone caption import, drag-`.txt`-onto-image | Working on image upload/move/copy/rename or caption import | ~1290 |
...
| `docs/dev/video.md` | `Video` model and `videos/` layout, `media_types.py` allowlist, cv2 metadata ladder and its duration guard, video ingest via upload/folder-import/rescan, the `/videos` endpoints and range serving | Working on videos, frame extraction, or any file-extension allowlist | ~1650 |
