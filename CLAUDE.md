...
| `docs/dev/scoring.md` | Working on `QualityPage`, quality flags, or thresholds — the scorers and the columns they write, duplicate detection, style similarity, DINOv2 per-layer scoring | ~2180 |
...
| `docs/dev/bulk-image-jobs.md` | Working on `CropToDetectionForm`, batch resize/crop, or the thumbnail rebuild — the bulk jobs that rewrite files: `detection_crop_rect`, the crop remap, `thumbnails_stale`, PM-013 loop ordering | ~2075 |
...
| `docs/dev/versioning-service.md` | Working on `version_service.py` or any path that overwrites or deletes an image file in place — `protect_file_before_overwrite`/`mark_image_deleted_in_versions` and their call sites, `create_snapshot`, restore's four passes, diff | ~2135 |
