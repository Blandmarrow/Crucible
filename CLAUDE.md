...
1. **NVIDIA** — skips if `torch.cuda.is_available()` is already True; otherwise checks for `nvidia-smi`, parses the `CUDA Version: X.Y` line, and installs `torch>=2.0` from the matching PyTorch wheel index (`cu128`, `cu126`, `cu124`, `cu121`, or `cu118`).
2. **AMD ROCm** (Linux only) — detected via `rocm-smi`; ROCm version determined via `rocminfo`, `/opt/rocm-*` dirname, or `/opt/rocm/VERSION` in that order; maps to wheel tag `rocm6.1`, `rocm6.2`, or `rocm6.3`. ROCm < 6.1 falls back to CPU. `manage.ps1` is unchanged (ROCm has no Windows support).
