...
- Filter by **keep/cut rating** — a chip row above the grid, multi-select and combined with **or**, with **Unrated** as one of the chips so "Keep or Unrated" is one click each. The number of unrated images in the dataset sits beside it, because a rating filter means little without it → [details](rating.md)
...
- **What a card shows** — the thumbnail carries the selection checkbox (top-left), a quality-flag cluster for NSFW, AI artifacts, duplicate, blurry, watermark, near-uniform and stale scores plus the keep/cut rating badge (top-right → [details](rating.md)), the aesthetic score (bottom-right), and a thin **style match meter** along the bottom edge. The meter's length is the image's percentile among this dataset's own style-similarity scores — the reading that means the same thing whichever embedding model produced them → [details](scoring.md#reading-the-score). Images with no style score show no meter; switch it off entirely in Settings → Gallery. Below the thumbnail: the filename, an optional license badge (off by default, Settings → Gallery), a generation-info button when the image carries metadata, the dimensions, and the caption
- Per-image detail view with metadata, caption editor, crop, upscale and LUT tools, and a **Source & License** panel showing the image's effective source, URL, license badge and attribution — each marked when it is inherited from the dataset rather than set on the image, with an edit button for overriding any of them ([details](provenance.md)); **keyboard shortcuts**: ← / → navigate between images, **Space** toggles selection, **1**–**4** set the keep/cut rating and **0** clears it ([details](rating.md)), **Delete** opens the delete confirmation. A **Select** button in the toolbar (checkbox icon) also toggles whether the current image is in the active selection. The caption editor shows a live **token counter** (word count · GPT-2 BPE token count) that turns amber at ≥ 70 tokens and red at ≥ 77 — the CLIP truncation limit.
...
The gallery sort dropdown covers newest/oldest, name, aesthetic score, style similarity, colourfulness, brightness, **Rating ↓ / ↑** (your keep/cut decision, best first — unrated sorts last in both directions → [details](rating.md)), and **Custom order** (below). Your choice is remembered per dataset, and **Settings → Gallery** sets the default for datasets you have not sorted yet.
