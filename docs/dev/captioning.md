...
- **Progress and failures**: each event carries `image_id`, `throughput_ips`, and `vram_used_mb` (sampled every 10 images; Ollama always 0; WD14 and OpenAI-compat always 0). Failed images accumulate in `failed_image_ids`; a `caption_summary` SSE event is emitted after the loop if any failed. Cancellation is checked at each image boundary via `job_queue.raise_if_cancelled(job_id)` (an in-memory set check, used in both `_run` and `_run_pipeline_job`); see `docs/dev/backend-infrastructure.md` § Job cancellation.
