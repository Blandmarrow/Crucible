...
| `docs/dev/versioning.md` | Dataset version control: snapshots, branches, copy-on-write object store (atomic `_store_object`), diff, restore, COW injection points, object-store prune/GC, dataset-busy guard (409 during versioning jobs) | Working on `VersionsPage`, branch/snapshot logic, `dataset_busy`, or any code path that overwrites/deletes image files in place | ~110 |
