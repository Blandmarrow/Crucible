...
- **A shallow merge preserves a stored value whose *type* has since changed**, which the `Array.isArray` note below covers for sets but not for scalars. `QUALITY_WORKFLOW_KEY`'s `dinoLayer` is the worked example: it was `number | "all" | null` and is now `number | "final" | "all"`, so a stored `null` survives the merge untouched and would render an empty `<select>`. The remedy is a **normaliser at the load boundary** — `normalizeDinoLayer` in `frontend/src/constants/styleModes.ts`, applied inside the `useState` initialiser — not a defaults change, which the merge would ignore. Two consequences are accepted rather than fixed: an existing user keeps whatever they stored while `SelectionToolbar`, which has no persistence, takes the new default immediately, so the two screens can disagree until *Reset to defaults*; and the normaliser has to decide what an old encoding *meant* rather than what it looked like (`null` becomes `"final"`, the vector it actually scored, not the new default). See `docs/dev/style-similarity.md` § The layer picker has three values, not two.
