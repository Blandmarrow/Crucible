...
| `docs/dev/video-extract-ui.md` | Working on the extraction modal or its progress rows — `ExtractFramesModal`'s two steps, `CropOverlay`/`TrimBar`/`NumberField`, `ExtractProgressList`, `useVideoExtractJobs` and the extraction re-attach | ~3250 |
