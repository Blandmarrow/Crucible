...
| `rating_filter` | `str` (JSON) | JSON-encoded **array** of keep/cut tiers `0`–`4`, where **`0` means unrated**, parsed by `utils.parse_rating_filter_param`. The tiers become one `OR` — `aesthetic_rating IN (…)` plus `aesthetic_rating IS NULL` when `0` is present — because the chip UI needs "some tiers *or* unrated", which the `license_filter`/`license_missing` pair's two independent clauses cannot express. That is the one place this table departs from the license precedent; `0` is unambiguous where `""` is not. Anything outside 0–4, a non-integer, or a non-list is a **400**, never a narrowed filter. See `docs/dev/rating.md` |
