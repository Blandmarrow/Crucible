...
- **Select all** in the toolbar selects the page you are looking at. When there is more behind your current filters than fits on that page, a line appears above the grid — *All 50 on this page selected — **Select all 1,240 matching filters*** — and taking it selects every matching image, across every page, without leaving the one you are on. It follows the filters exactly: narrow to a subfolder, a license, or one video's frames first and the offer counts only those. Anything you then do from the selection toolbar applies to the whole set, and a destructive action confirms with the real number. Very large selections stop at 20,000 images; you will be told when that happens, rather than quietly getting fewer than you asked for
