...
- **Every path that rewrites an image's pixels in place goes through `utils.record_in_place(img, op, **params)`.** It is the single writer of `Image.processing_history` — pass 2's re-extraction skip guard — *and* of two staleness bits: `Image.scores_stale`, saying the ten `*_score` columns and the `quality_flags` derived from them were measured on pixels that no longer exist, and `Image.rating_stale`, saying the same about the human's `aesthetic_rating`. Each bit is written only for a row that carries the thing it qualifies — an unscored row has no measurement to invalidate and an unrated one no judgement — while the history entry is written either way. Two writers drift, and the failure is silent in the worse direction: a site that records the edit by hand leaves the scores looking trustworthy, and `exclude_flags` then drops images at export on flags computed against a deleted image. A structural AST guard in `backend/tests/test_scores_stale.py` fails CI for any `processing_history = … + …` list-concat outside `backend/utils.py`. Ten sites today: batch/single resize, batch/single crop, `crop_upscale`, LUT, upscale, crop-to-detection, re-extraction. **The two bits are separate because their clear predicates diverge**: `scores_stale` is cleared **only** by a `routers/quality.py` run that **actually measured something** and refreshed every score the row carries, while nothing but a human re-rating clears `rating_stale` (sole site: `POST /images/bulk-rating`) — one column would let a re-score declare a stale judgement current. See `docs/dev/scores-stale.md`, `docs/dev/rating.md` and PM-010.
...
|---|---|---|
| `docs/dev/shared-utilities.md` | Importing anything from `backend/utils.py`, `media_types.py` or `licenses.py` — path guards and containment, naming and collisions, caption sidecars, `record_in_place`, client-supplied regex, batching, the ingestible-extension allowlist, the license vocabulary, the shared frontend components | ~1405 |
...
| `docs/dev/scores-stale.md` | Working on the `scores_stale` bit, an in-place pixel rewrite, or the re-score clear predicate — `record_in_place` as single writer, `score_columns`, `_JOB_SCORE_COLUMNS`, the badge/chip/export surfaces | ~1590 |
| `docs/dev/rating.md` | Working on the keep/cut rating, the rating event log or the Aesthetic Rating page — `aesthetic_rating`/`rating_stale`, the scale direction, the two-bits argument, travel and derivative rules, `rating_filter`'s departure from the license precedent, `bulk-rating`, the export include/exclude pair and its unrated advisory, the restore-revert count, `image_rating_events` and its restore non-invariant, the self-agreement ceiling and the scorer-agreement metrics, the page's three sample-size floors, why the page's e2e asserts deltas | ~3665 |
| `docs/dev/image-similarity.md` | Working on duplicate detection, pHash grouping, bulk duplicate resolution, style similarity, the style-run descriptor or the style-match meter — `find_duplicates_sync`'s two implementations, same-source duplicate groups, the duplicates panel's filter chips/bulk bar/batched resolve, the `embedding_type` modes and what each is measurably worth, all-layers DINOv2 scoring, `StyleSimilarityRun` and `GET /quality/style-similarity/{id}`, the percentile contract, the card meter and the detail block | ~4745 |
| `docs/dev/gallery.md` | Working on `GalleryPage`, the subfolder sidebar, or a gallery filter — selection and shift-click ranges, select-all-matching-filters, subfolder create/rename/move/delete and the row context menu, expand/collapse and its persistence, filters (including the rating chips), pagination, sorting | ~5475 |
...
| `docs/dev/image-detail.md` | Working on `ImageDetailPage`, the style-match block or generation-metadata display — gallery/nav persisted keys, `injectNavId`/`paneGo`, crop tool, selection toggle, caption panel, `StyleMatchPanel` and `DinoLayerBreakdown` | ~2355 |
...
| `docs/dev/export.md` | Working on `ExportPage` or `export_service.py` — kohya/ai-toolkit/plain, the shared export loop, stem uniquification, the filter table and preview, resize, loss masks, captions-only | ~2350 |
...
| `docs/dev/versioning-service.md` | Working on `version_service.py` or any path that overwrites or deletes an image file in place — `protect_file_before_overwrite`/`mark_image_deleted_in_versions` and their call sites, `create_snapshot`, restore's four passes, diff | ~2410 |
...
| `docs/dev/statistics.md` | Working on `StatsPage`, `BucketPanel`, or `get_dataset_stats` — stats queries and live polling, server-side aggregation, the validator-keyed cache, editable histograms | ~2765 |
| `docs/dev/image-filters.md` | Adding or changing a `GET /images/` filter param — the shared listing contract behind the gallery filter bar, stats click-through, frame lineage and the license filters, plus `GET /images/count` and `/images/ids` and the `ImageFilterParams` model all three share | ~1425 |
| `docs/dev/settings.md` | Working on `SettingsPage`, a new app-wide setting or secret, `threshold_service.py` or `secrets_service.py` — the `ThresholdSettings` singleton row and every tab it backs, the DB-over-`.env` secret precedence and the `HF_TOKEN` projection | ~2375 |
...
| `docs/dev/frontend-core.md` | Working on global frontend state, a shared constants module, or the JS error console — TanStack Query/Zustand conventions, the `SelectionToolbar` action modals, `uploadStore` | ~2520 |
...
| `docs/dev/panes-routing.md` | Working on panes, adding a routed page, or lazy page loading — sidebar layout, the split-view pane manager, `usePaneNavigate`, the eight-site routed-page checklist | ~1245 |
| `docs/dev/persistence.md` | Adding a storage key or persisting page configuration — the `constants/storage.ts` key registry, `loadPersisted`/`useDebouncedPersist`, the three persistence shapes | ~2205 |
| `docs/dev/styling.md` | Working on Tailwind/CSS, the brand mark, any modal dialog, or the shared right-click menu — CSS variable tokens, `@layer components` classes, `CrucibleMark` drift checks, `ConfirmDialog`, `useModalBehavior`, `ContextMenu` | ~1930 |
