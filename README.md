...
- **Rename and re-nest** a gallery subfolder from its right-click menu or by dragging one row onto another — the whole subtree follows, and the files inside keep their names → [details](docs/gallery.md#renaming-and-re-nesting-a-subfolder)
...
### Quality & curation
- **Score** every image across aesthetic, technical, watermark, NSFW, and style similarity metrics — aesthetic scoring offers a choice of two models (LAION over CLIP, or Aesthetic Predictor V2.5 over SigLIP), and records which one produced each score. Style similarity ranks a dataset against reference images and reads as a **percentile** on every gallery card and on the image detail page, since the raw cosine's scale depends on which embedding model, layer and blend produced it → [details](docs/scoring.md) · [style similarity](docs/style-similarity.md)
