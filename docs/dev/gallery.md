...
- **`VideoStrip` is mounted outside this `DndContext`** (and outside the grid's scroll container) on purpose: inside it, the strip's cards would join the collision detection above and the subfolder droppables, and inside the container they would sit under the drag-to-upload handler. See `docs/dev/video.md` § Frontend.
