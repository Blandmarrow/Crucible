...
- Editable histogram bucket edges — rebucketing runs entirely client-side against raw score arrays. Edited edges are remembered per dataset and per panel, so a panel whose defaults have since been corrected keeps showing your saved ones — the Saturation panel's defaults changed to the 0–1 scale it is actually measured on. Use **Reset** in the panel's edit popover to pick the corrected defaults back up
- Top-20 tag frequency chart (the full 500 are in the tag CSV) and tag co-occurrence matrix
- The **Summary** section includes a score guide table (metric, value range, flag threshold, detection method) and score coverage bars showing what percentage of images have been scored for each metric. When a dataset's aesthetic scores came from more than one model, the Aesthetic bar carries a breakdown of the split and a note that the scales are not comparable → [details](scoring.md#choosing-the-aesthetic-model)
