...
| `docs/dev/scoring.md` | Working on `QualityPage`, quality flags, thresholds, or the aesthetic model picker — the scorers and the columns they write, the failure contract when nothing can be measured, `aesthetic_model` and its two consumer guards, per-model coverage, the flag thresholds, e2e coverage | ~2605 |
...
| `docs/dev/image-similarity.md` | Working on duplicate detection, pHash grouping, bulk duplicate resolution, or style similarity — `find_duplicates_sync`'s two implementations, same-source duplicate groups, the duplicates panel's filter chips/bulk bar/batched resolve, the `embedding_type` modes and what each is measurably worth, all-layers DINOv2 scoring | ~3195 |
| `docs/dev/gallery.md` | Working on `GalleryPage` or gallery drag & drop — selection and shift-click ranges, select-all-matching-filters, subfolder sidebar, filters, pagination, manual ordering, dnd-kit droppables and collision detection | ~4200 |
