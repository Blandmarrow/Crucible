...
- **Gallery defaults** section: default sort (`GALLERY_DEFAULT_SORT_KEY`, index into `SORT_OPTIONS`), default caption filter (`GALLERY_DEFAULT_CAPTION_KEY`, `"all" | "captioned" | "uncaptioned"`), default quality filter (`GALLERY_DEFAULT_QUALITY_KEY`, flag key or `""`). Applied by `GalleryPage` on first visit to a dataset; session state (`gallery-state-*`) takes precedence. Helpers `getGalleryDefaultSort()`, `getGalleryDefaultCaptionFilter()`, `getGalleryDefaultQualityFilter()` in `storage.ts` are used at `useState` init time.

**Captioning tab** — seven immediate-save preferences (lazy-loads `["captioning-models"]` query only when tab is first opened):
- Default model (`CAPTION_DEFAULT_MODEL_KEY`). Applied once by `CaptioningPage` via `useEffect` when `modelsData` loads and `selectedModel` is still `""`. Falls back to no selection if the stored model is not installed. Also corrects the stored style if it is incompatible with the applied model (sets it to the first supported style for that model type).
