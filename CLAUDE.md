...
| `docs/dev/gallery.md` | Working on `GalleryPage`, the subfolder sidebar, or a gallery filter — selection and shift-click ranges, select-all-matching-filters, subfolder create/rename/move/delete and the row context menu, filters, pagination, sorting | ~4335 |
| `docs/dev/gallery-dnd.md` | Working on gallery drag & drop — manual `sort_order` ordering, dragging image cards onto subfolder rows, dragging a subfolder onto another, dnd-kit droppable/draggable id namespaces, collision detection and the drag overlay | ~2385 |
