...
| `docs/dev/video.md` | **Start here for anything video** — its § Where things live table routes from a code path to whichever of the eleven sibling docs owns it. Also covers the `Video` model and `videos/` layout, poster stems and collisions, the three ingest paths, the two `Dataset` columns | ~2260 |
...
| `docs/dev/video-tests.md` | Adding a video test, a cv2-gated test, or a CI media dependency — the arc's test index, the four `mp4_*` fixtures and their constraints, branches no fixture reaches, cv2/scenedetect in CI, the skip convention | ~1345 |
