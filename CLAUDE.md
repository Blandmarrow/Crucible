...
- `within_datasets_dir(path_str, base_dir) -> Path | None` / `safe_dataset_path(...) -> Path` — resolve a stored path, returning it only if `is_relative_to(base_dir)` — under a string prefix, `/data/datasets_backup` passed a `/data/datasets` base. **Serve** routes take the raising wrapper (403); **destructive** ones take the `None`, skip the filesystem op and still drop the row, and the gate covers the versioning hook, which *reads* the file. Never re-inline the check.
...
| `docs/dev/image-detail.md` | Gallery/nav persisted keys, `injectNavId`/`paneGo`, crop tool, selection toggle, caption panel, AI generation metadata extraction and display | Working on `ImageDetailPage` or generation-metadata display | ~1755 |
...
| `docs/dev/versioning.md` | Snapshots, branches, copy-on-write object store (atomic `_store_object`), diff, restore, COW injection points, object-store prune/GC, the dataset-busy 409 guard | Working on `VersionsPage`, `dataset_busy`, or any path that overwrites image files in place | ~3095 |
...
| `docs/dev/workspace.md` | Sidebar hardware meters and `/system`, file browser and `/filesystem`, Logs page (job history + JS error console), Booru tag lookup | Working on hardware meters, `FileBrowserPage`, `LogsPage`, or `BooruPage` | ~2745 |
...
| `docs/dev/panes-routing.md` | Sidebar layout, split-view pane manager, `usePaneNavigate`, route-level code splitting and the six-site routed-page checklist | Working on panes, adding a routed page, or lazy page loading | ~955 |
...
| `docs/dev/postmortems.md` | Postmortem index: past incidents as one-line rows (symptom, root-cause category, LIVE/MITIGATED/STRUCTURAL status), linking detail files under `docs/dev/postmortems/` | Doing a code review or investigating a bug — check the code under review against known failure classes | ~850 |
