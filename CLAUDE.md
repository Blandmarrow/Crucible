...
| `docs/dev/rating.md` | Working on the keep/cut rating — `aesthetic_rating`/`rating_stale`, the scale direction, the two-bits argument, travel and derivative rules, `rating_filter`'s departure from the license precedent, `bulk-rating`, the export include/exclude pair and its unrated advisory, the restore-revert count | ~1720 |
