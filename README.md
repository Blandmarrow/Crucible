...
- **Hold videos** alongside a dataset's images as sources for frame extraction — stored and counted separately, browsable in a strip above the image grid with poster frames, an inline player, rename and delete (experimental; frame extraction is not built yet) → [details](docs/gallery.md)
