...
| `docs/dev/settings.md` | Working on `SettingsPage`, a new app-wide setting or secret, `threshold_service.py` or `secrets_service.py` — the `ThresholdSettings` singleton row and every tab it backs, the DB-over-`.env` secret precedence and the `HF_TOKEN` projection | ~2115 |
