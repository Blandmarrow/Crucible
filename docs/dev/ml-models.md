...
**Duplicate detection** (`technical_scorer.find_duplicates_sync`) runs after technical scoring: it greedily groups images whose phash Hamming distance is `< duplicate_threshold` (first unassigned image is the group root; each other image is claimed once). The implementation is vectorized with numpy — hex phashes are decoded once into an `(N, L)` uint8 array and each row's distance to all later rows is one op via a module-level 256-entry popcount table (`POPCNT`), length-generic so it doesn't assume 64-bit hashes. Grouping semantics are identical to the old scalar `imagehash` loop (verified byte-for-byte). The consumer `_flag_duplicates` (`routers/quality.py`) then loads the flagged images with a single chunked `select(...).where(Image.id.in_(...))` (≤10k ids per chunk) rather than per-row `session.get`, and follows the copy-then-reassign `quality_flags` invariant.
