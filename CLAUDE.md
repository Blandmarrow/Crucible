...
- **Path traversal guard.** No stored path becomes a `FileResponse` or gets mutated until the guards above prove it inside `settings.datasets_dir`, and no router keeps a private copy. Seven destructive sites gate **every** column they touch — `file_path`, `thumbnail_path`/`poster_path`, the derived `.txt` — and unlink the *resolved* path, never the raw string: `delete_image`, `batch_delete`, `bulk_delete_filtered`, `delete_video`, `rename_video`, plus `filesystem.delete_path` (via `_add_orphan`) and `filesystem.rename_path`'s thumbnail move, which PM-014 added. **Three sites do not, and are known-open** (V-83): `videos._delete_previous_frames` (replace-mode extraction), `quality.py`'s duplicate resolution, and `version_service._delete_image_files` all `unlink` a raw stored string. Their row sets are scoped by `dataset_id`/`source_video_id` rather than by a client-supplied path, which is why they are a filed finding and not a HIGH — but this list is a reviewer's checklist, so a new destructive path belongs in the gated group, and the ungated three are not a precedent to copy.
...
| `docs/dev/backend-infrastructure.md` | Working on `main.py` lifecycle, Alembic migrations, SSE, or job cancellation — production frontend serving, the shutdown/restart loop, DB indexes and deferred columns, the SSE broadcaster | ~2820 |
