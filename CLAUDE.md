...
| `docs/dev/video-extract.md` | Frame extraction: probe sampling and its RSS rule, the PySceneDetect contract and cost cliff, shot rendering, `POST /videos/probe` and `/videos/extract`, the `video_extract` job | Working on frame extraction, shot detection, or the extract/probe endpoints | ~2870 |
