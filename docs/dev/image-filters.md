...
| `label_missing` | `bool` | `true` = only images carrying no label at all (`~exists`); `false` = only images that carry one. Combining `true` with a non-empty `label_filter` is a **400** — unsatisfiable, and a query that always returns zero rows is indistinguishable from a broken filter |
