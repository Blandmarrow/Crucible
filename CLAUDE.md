...
| `docs/dev/labels.md` | Working on the `/labels` router, the label filters, the Settings Labels tab, or the detail-view hotkeys — the global vocabulary and `image_labels` join table, `label_service.py`, `POST /labels/assign`, the EXISTS-only filter block, the `VersionImageState.label_ids` mirror, the frontend surfaces | ~2690 |
| `docs/dev/image-filters.md` | Adding or changing a `GET /images/` filter param — the shared listing contract behind the gallery filter bar, stats click-through, frame lineage and the license filters, plus `GET /images/count` and `/images/ids` and the `ImageFilterParams` model all three share | ~1515 |
