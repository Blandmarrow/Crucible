...
- **Live export preview** — shows exact will-export and excluded counts (broken down by filter reason) before you run, and warns when any images have no license recorded, when free-text licenses are slipping past the no-derivatives filter, or when images were edited in place after they were scored
- **Stale-score warning** — resizing, cropping, upscaling, grading or re-extracting an image does not re-score it, so its quality scores and flags describe pixels that are gone. The preview counts those images and says how many the current filters will actually ship. It matters here because *per-flag exclusions* decide what to ship from exactly those flags, so a stale run can drop images worth keeping. Re-run scoring to clear it → [details](scoring.md#stale-scores). Advisory only — the export never blocks
- **Mixed aesthetic models** — when a dataset's aesthetic scores were produced by more than one model, the preview reports the split, because the two scales are not comparable. If a minimum-aesthetic filter is on, it also says that one threshold is being applied to both. Advisory only, and shown whether or not you are filtering, since the gallery's aesthetic sort reads the same column → [details](scoring.md#choosing-the-aesthetic-model)
- **Loss masks** — see below

The output folder must be an **absolute** path (`D:\datasets\out`, `/home/me/out`). The **Browse…** picker always produces one; a relative path typed in by hand is rejected before the export starts.
