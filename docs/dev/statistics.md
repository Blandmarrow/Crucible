...
This file covers `StatsPage`: its queries and live polling, server-side stats aggregation and the `DatasetStats` schema, editable histograms, CSV export, bar click-through into `BucketPanel`, and the Detections & Masks and Licenses panels. The `GET /images/` params those clicks send are `docs/dev/image-filters.md`.
...
**Validator-keyed stats cache** (`dataset_service.py`, `_stats_cache` + `_stats_cache_get`/`_stats_cache_put`/`_image_validator`): both functions above pull *every* image row in scope into Python, and StatsPage live-polls them, so an idle Stats tab re-read the whole table every few seconds. Each call now first runs a cheap validator query — `SELECT COUNT(id), MAX(updated_at)` under the same dataset/subfolder filter, plus `Dataset.updated_at` for `get_dataset_stats` (whose `license_breakdown` coalesces over the dataset default) — and serves the previous payload when the validator is unchanged. Keys are `(dataset_id, subfolder, "stats" | "scores" | "style")`; the cache is a module-level dict capped at `_STATS_CACHE_MAX` (64) entries, dropping oldest-inserted. Staleness is bounded to writes that leave `(count, max updated_at)` identical, which no ORM or Core write can do — `Image.updated_at` carries an `onupdate`. Two consequences for callers: the returned dict **is** the cached object, so never mutate a stats/score-values payload in place; and a new column that affects the payload but lives on `Dataset` (not `Image`) needs `Dataset.updated_at` to be bumped when it changes, or the cache will not notice. Covered by `backend/tests/test_stats_cache_http.py`. SQL-side bucketing was considered and rejected — it would duplicate the thirteen metrics' bucket-edge definitions that already exist as `DEFAULT_EDGES` in `StatsPage.tsx`, and would not help `get_score_values` at all.

**The third slot, `"style"`, and what it costs.** `get_style_distribution` (see
`docs/dev/image-similarity.md`) caches here too. With three payload kinds per scope rather
than two, `_STATS_CACHE_MAX`'s 64 entries covers a working set of about 21 datasets instead
of about 32. That is the accepted cost and **not** a reason to raise the constant: the style
payload is ~200 bytes against the other two's per-row arrays, so the entries it evicts are
the cheapest in the cache by a wide margin. Its validator is `_style_validator` rather than
`_image_validator` — the image half plus the dataset's `StyleSimilarityRun.updated_at`,
because that payload embeds a row from another table. It is also the one slot with no
subfolder dimension, always keyed `(dataset_id, None, "style")`, so an image reads the same
percentile in a filtered pane as in an unfiltered one.
...
**Scope: images only.** `DatasetStats` carries no video field and `get_dataset_stats` scans `images` alone, so a dataset holding 40 videos shows nothing about them anywhere on this page — the counts, every histogram, and `total_size_mb` (which falls back to `ds.total_size_bytes`, itself images-only by design) all describe the dataset's images. `Dataset.video_count`/`video_size_bytes` are separate columns, surfaced on the dataset card rather than here; see `docs/dev/video.md` § Stats.
...
See `docs/dev/image-similarity.md` for the style-similarity scoring flow and embedding types that populate `style_similarity_distribution`.
...
**Editable histograms (HistPanel)**: Every score/metric histogram has a pencil icon that opens an inline edge editor. The user types comma-separated boundary values (e.g. `"4, 6"` for aesthetic score), presses Apply or Enter, and the chart immediately rebuckets using the raw value arrays. A "custom" badge appears in the panel title when non-default edges are active; Reset restores the defaults. Aspect ratio and file format histograms are non-editable (no raw values to rebucket). When a customised bar is clicked, `BucketPanel` still opens with the correct `min`/`max` filter derived from the custom edges. `HistPanel` accepts an optional `storageKey?: string` prop; when provided, the active edge string is persisted to that `localStorage` key on change and restored on mount (with a second `useEffect` that re-reads the key when `storageKey` changes to handle dataset switches). Score histograms pass a literal `stats-hist-edges-{metric}-${datasetId}` key — thirteen of them — so custom bucketing survives navigation and one metric's edges never leak to another. **Not** derived from `STATS_FILTERS_PREFIX`, which is the filters blob alone; these are a fourth class of component-local key (`docs/dev/persistence.md`).
