...
- **Drag-and-drop assignment**: when `hasAnyCategory` is true all dataset cards become draggable. `onDragStart` uses `dataTransfer.setData("dataset-id", id)` to distinguish from file-upload drags (which use the `"Files"` type). `renderCategorySection` attaches React `onDragOver`/`onDragLeave`/`onDrop` handlers; `onDragOver` guards with `if (dropTargetCategory !== cat)` to avoid redundant re-renders. Dropping on a named section calls `moveCategoryMutation` (PATCH `dataset.category`); dropping on "(Uncategorized)" sets `category: ""`. Empty sections show a dashed placeholder that becomes the visual drop target.
