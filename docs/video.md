...
Tick the checkbox on a card to select it; shift-click a second card to select the run between them. With anything selected the strip header offers **Extract frames**, which runs the same settings across every selected video at once, and **Delete**, which removes the selected videos after a confirmation. The **Delete** key does the same — except while gallery images are also selected, where it keeps its usual meaning and deletes those instead, so one keypress can never mean two things. **Clear** drops the selection, and it clears itself when you switch datasets. If a delete fails — a file open in another program, say — that video stays selected and the reason is shown, so you can close the program and press Delete again.
...
**Some videos will not play in the browser even though Crucible can read them.** Browsers ship a short list of decoders, and 10-bit H.264, HEVC/H.265, ProRes and the `.mkv` and `.avi` containers are outside it — Firefox reports this as *"the file is corrupt"*, which it is not. Where Crucible can tell in advance, a note appears under **Codec** in the video's panel; if playback fails anyway, a message over the poster explains it. That message only claims the file is fine when Crucible predicted the failure from the file itself — otherwise it says both possibilities, since a browser reports a file that has been renamed, moved or deleted exactly as it reports a missing decoder. **Everything else still works on those files**: the poster, sampling, shot detection and frame extraction all read the video directly rather than through the browser, so an unplayable clip extracts exactly like any other.

**Delete video** removes the video and its poster. Frames already extracted from it are ordinary images and are deliberately left alone — the confirmation says how many there are; they keep their files and only lose the link back to the video. **Delete** on the keyboard opens the same confirmation. If the file is open in another program — a media player, say — the delete is refused with a message instead of half-succeeding; close it and try again. Renaming behaves the same way.
