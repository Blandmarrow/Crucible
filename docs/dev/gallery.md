...
- **Rating filter** — a chip row, not a dropdown, and the only multi-select filter here: the chips are the four tiers plus **Unrated**, combined as OR into one `rating_filter` JSON array where `0` is the unrated sentinel. State is `ratingFilter: number[]`, sanitised to 0–4 on restore (an out-of-domain entry would be a 400 from the listing endpoint, not an empty grid), persisted in `gallery-state-${datasetId}` alongside `licenseFilter`, and cleared by **Reset filters**. The dataset's unrated total rides beside the chips from its own `["images", datasetId, "count", "unrated"]` query — deliberately whole-dataset rather than filter-scoped, so it does not move as chips are clicked. Keys `1`–`4`/`0` rate the current selection from a window `keydown` that guards on pane, `[role="dialog"]` and text fields. See `docs/dev/rating.md`.
...
**Rating ↓ / ↑** were appended for the same reason and with the same constraint. Both directions this time — "show me what I marked Cut" is a real review pass — and `aesthetic_rating` joins `_NULLS_LAST_SORT_FIELDS` server-side so unrated sorts last either way; it is deliberately **not** in `_ALLOWED_SCORE_FIELDS`, which also drives the score filters and the Stats histograms. See `docs/dev/rating.md`.
