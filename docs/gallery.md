...
- Each dataset card and compact row shows its default license as a colour-coded badge, so you can see at a glance which datasets are commercially usable — and which still say *No license*
...
- Filter by **license** — any single license, or **Missing license only** to find images with no license at either the image or dataset level → [details](provenance.md)
