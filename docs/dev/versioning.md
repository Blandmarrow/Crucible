...
- `_backup_and_record_hash(image_id, file_path, dataset_folder, db, precomputed_sha256=None)` — shared helper: hash → copy to object store → backfill all NULL `file_hash` rows for the image in one `UPDATE`. `precomputed_sha256` skips the hashing pass when the caller already hashed the file (restore hashes each file at most once and passes the result through `protect_file_before_overwrite`).
...
- `restore_snapshot(db, dataset_id, version_id, handle_extra_images, pre_restore_snapshot, job_id)` — optionally auto-snapshots current state first; after all files are restored, sets `branch.head_version_id = version_id` so the UI "Current" badge moves to the restored snapshot. Runs as **four passes** (plan → protect → DB-commit → execute), each with an invariant:
  - **Pass 0/1 — gated, hash-once COW protection (undoable in auto mode)**: `protect_file_before_overwrite` fires *only* for files the restore will actually overwrite/move/delete (in-place restores hash the file once, decide `needs_restore`, and reuse the hash via `precomputed_sha256`; pure renames are moved, never backed up), so the pre-restore snapshot's NULL-hash rows get backfilled without hashing/copying the whole dataset. Extras occupying a restore target are protected too (`keep` mode) or run through `mark_image_deleted_in_versions` (`remove` mode) before any file write. Manual mode is unaffected (its pre-restore snapshot copies everything eagerly).
  - **Pass 2 — DB before filesystem**: all Image rows (re-creations, metadata, and the reverted `filename`/`original_filename`/`file_path`) are committed *before* any file moves, matching the CLAUDE.md DB-before-filesystem invariant; a move that later fails gets a per-image compensating fixup (`_compensate_paths`) pointing the row back at the file's actual location.
  - **Pass 3 — collision-safe execution**: rename sources that are another image's restore target are first staged to `{stem}.__restore_tmp_{id}{suffix}` temp names (swaps/renumber chains can't clobber a file that is still someone's current content). Content restores copy from the object store to `target_path`; a pure rename (auto mode leaves `file_hash=NULL`) *moves* the file (`rename_with_sidecar`). Stale files/sidecars/thumbnails at the old path are removed **only after the restore/move succeeded** — and when the object-store copy is missing, the current file is *moved* to the target, never deleted (it may be the only remaining copy; counted in `files_unavailable`). All unlinks go through `_remove_stale_files` via `run_in_executor`. Thumbnails are regenerated from the row's final `file_path`. Finally, `_sync_caption_sidecar` makes the on-disk `.txt` sidecar match the restored `caption_text` for every image (written when non-empty, deleted when empty) — without this, `rescan_dataset` treats the stale sidecar as a caption edit and silently reverts the restored caption seconds later when `auto_rescan_on_open` fires.
- `diff_versions(db, dataset_id, v1, v2)` — pure DB, no background job; uses `_DIFF_COLS` column-explicit select for efficiency. Compared fields: `caption_text`, `quality_flags`, `subfolder`, all seven quality scores (incl. `color_score`), `dino_layer_scores`, `generation_metadata`, `sort_order`, `processing_history`, plus `file` (hash). The heavy JSON fields in `_HEAVY_DIFF_FIELDS` (`dino_layer_scores`, `generation_metadata` — full ComfyUI workflow payloads) are compared but reported only as a compact `{"changed": true}` marker, never embedded. `processing_history` changes render as `+`/`−` operation badges in `DiffModal`; `{"changed": true}` renders as "field: changed"; the generic `ChangeRow` renders any other field via `JSON.stringify`.
